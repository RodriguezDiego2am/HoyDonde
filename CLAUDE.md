...
## Reports module — Organizer events report (first backend cut, implemented — see docs/api-mvp-plan.md §11)

Only the Organizer's own-events report is implemented. The Admin global report, the security-audit report, and any frontend/PDF are not — do not describe them as implemented.

- `Authorization/Acciones.cs` goes from 20 to 22 actions: `REPORTE_VER_GLOBAL` (reserved for the future Admin report) and `REPORTE_VER_PROPIO` (used below). `SecurityCatalogSeeder` assigns them to `ADMINISTRADOR`/`ORGANIZADOR` respectively, but **only for new installations** (dev/test/emulator) — against real Firestore, `SecurityCatalogSeeder.SeedAsync()` never runs again once an effective Administrator exists.
- For the real, already-provisioned Firestore project, a dedicated command creates just the two `Accion` documents, idempotently, and touches nothing else: `dotnet run --project HoyDonde.API -- seed-report-actions`. It never creates/edits a `Rol`, never assigns an `Accion` to a `Rol`, and re-running it never restores an assignment an Administrator revoked — the Administrator assigns these actions to whichever roles they choose from `/api/security` afterward.
- `GET /api/reports/organizer/events` (Policy `REPORTE_VER_PROPIO`): `fechaDesde`/`fechaHasta` required (UTC explicit, `fechaDesde` inclusive, `fechaHasta` exclusive, max 366 days — `REPORT_RANGE_INVALID` → 400 otherwise), plus optional `estado`/`categoria`/`eventId`/`ticketTypeId` (`ticketTypeId` requires `eventId` — `REPORT_FILTER_INVALID` → 400 otherwise). Never accepts `organizadorPersonaId`: the organizer always comes from `IAuthenticatedPersonaResolver`.
- Ownership is always part of the Firestore query itself (`WhereEqualTo(OrganizadorPersonaId, actorPersonaId)` combined with the `FechaInicio` range), never only an in-memory filter applied after reading. A caller-supplied `eventId` outside the organizer's own events is `EventOwnershipException`/`EventNotFoundException` (403/404, same codes as `EventService`); an owned event outside the requested range/estado/categoria yields an empty report, never a data leak.
- Tickets for the resulting events are read via `WhereIn(EventoId, chunk)` in batches of **at most 30** (Firestore's real limit) — never one read per event; if there are no matching events, no ticket query runs at all. This is a non-transactional, informational read: a concurrent purchase during report generation may or may not be reflected depending on the exact snapshot moment.
- The DTOs (`ReporteEventosResponseDto`/`ReporteResumenDto`/`ReporteEventoDetalleDto`/`ReporteTicketTypeDetalleDto`) never expose `OrganizadorPersonaId`, the Firebase UID, `UsuarioId`, or `ExternalSubjectId`. The monetary figure is always called **"importe emitido"**, never "recaudación"/"cobrado"/"ganancia" (the MVP does not process real payments); it is the sum of each ticket's immutable `PrecioPagado` photograph. Per-event capacity comes from `Event.CapacidadMaxima`; per-ticket-type capacity is a derivation (current `CantidadDisponible` + tickets already issued of that type), never a persisted field. Every percentage divides-by-zero to `0`, never an exception.
- New composite index `events: OrganizadorPersonaId ASC, FechaInicio ASC` added to `firestore.indexes.json` and exercised against the Firestore Emulator — **not yet deployed** against the real Firebase project (`hoydonde-f5a05`).
- **Status: verified against the Firestore Emulator only — 468 passed, 0 failed, 0 skipped (full suite).** Deploying the new index, running `seed-report-actions` against real Firestore, the Admin report, the security-audit report, and any frontend/PDF work all remain pending.
