...
- `Authorization/Acciones.cs` has 22 actions: `REPORTE_VER_GLOBAL` (Admin's two reports) and `REPORTE_VER_PROPIO` (Organizer's report). `SecurityCatalogSeeder` assigns them to `ADMINISTRADOR`/`ORGANIZADOR` **only for new installations** (dev/test/emulator); against real Firestore, `SecurityCatalogSeeder.SeedAsync()` never runs again once an effective Administrator exists — the dedicated `seed-report-actions` command (idempotent, creates only the two `Accion` documents, never touches an assignment) remains the real-Firestore path.
