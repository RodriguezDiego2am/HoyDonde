...
/**
 * Reporte de ventas simuladas del Organizador (docs/api-mvp-plan.md §11, GET
 * /reports/organizer/sales). A diferencia del reporte de desempeño (Compra.FechaCompra, no
 * Event.FechaInicio): responde "cuándo se vendió", no "cuándo ocurre el evento".
 */
export default function OrganizerSalesReportScreen() {
  ...
  const [selectedEvent, setSelectedEvent] = useState<EventOption | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    eventService
      ...
      .catch(() => {
        // El selector de evento es una comodidad del filtro: si falla, el reporte sigue funcionando sin él.
      })
      .finally(() => {
  ...
  const runReport = useCallback(async (filters: AppliedFilters) => {
    ...
    setError(null);
    try {
      const data = await reportService.getOrganizerSalesReport({
      ...
      // El evento pedido dejó de tener Compras dentro del rango/categoría vigentes (p. ej. se
      // cambió el rango o la categoría sin cambiar el evento seleccionado): ya no aparece entre
      // filtrosDisponibles.eventos, así que se limpia evento + tipo de entrada (nunca queda una
      // selección fantasma) y se vuelve a consultar sin acotar a ese evento.
      if (filters.eventId && !data.filtrosDisponibles.eventos.some((e) => e.id === filters.eventId)) {
        setSelectedEvent(undefined);
        setSelectedTicketTypeId(undefined);
        const sinEvento: AppliedFilters = { ...filters, eventId: undefined, eventNombre: undefined, ticketTypeId: undefined };
        setApplied(sinEvento);
        await runReport(sinEvento);
        return;
      }
  ...
  const handleApply = () => {
    ...
    const filters: AppliedFilters = {
      ...
      eventNombre: selectedEvent?.nombre,
  ...
  const filtrosActivosLabel = useMemo(() => {
    ...
    if (applied.ticketTypeId) {
      const tipo = report?.filtrosDisponibles.tiposEntrada.find((t) => t.id === applied.ticketTypeId);
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const html = wrapReportDocument({
        ...
        title: 'Reporte de ventas simuladas',
      ...
      const result = await generateAndShareReportPdf(html, 'Reporte de ventas simuladas');
      if (!result.shared) {
  ...
  return (
    <View style={styles.container}>
      <EditorialHeader eyebrow="ORGANIZACIÓN" title="Ventas simuladas" subtitle="Cuándo y cuánto se vendió." showBack onRefresh={applied ? handleRefresh : undefined} />
        ...
        <Surface style={styles.card}>
          <View style={styles.dateRow}>
            <View style={styles.dateField}>
              <SegmentedDateField label="Desde" value={fechaDesdeDraft} onChange={setFechaDesdeDraft} testIDPrefix="ventas-fecha-desde" />
            </View>
            <View style={styles.dateField}>
              <SegmentedDateField label="Hasta" value={fechaHastaDraft} onChange={setFechaHastaDraft} testIDPrefix="ventas-fecha-hasta" />
            </View>
          </View>
          {filterError ? (
          ...
          {selectedEvent && report && applied?.eventId === selectedEvent.id ? (
            report.filtrosDisponibles.tiposEntrada.length > 0 ? (
              <>
                <Text style={[styles.fieldLabel, styles.fieldLabelSpaced]}>Tipo de entrada (opcional)</Text>
                <ChipSelectRow
                  options={report.filtrosDisponibles.tiposEntrada.map((t) => ({ value: t.id, label: t.nombre }))}
            ...
            <Text style={styles.hintText}>Aplicá el filtro de evento para ver los tipos de entrada disponibles.</Text>
          ) : null}
        ...
        <View style={styles.backdrop}>
          ...
          <View style={styles.sheet}>
            ...
            <ScrollView style={styles.sheetList}>
              ...
              {report ? (
                // Con un reporte ya generado, las opciones vigentes son las que efectivamente
                // tienen Compras dentro del rango/ownership/categoría aplicados (docs/api-mvp-plan.md
                // §11.11) — nunca la lista completa de eventos propios, que puede incluir eventos
                // sin ninguna venta en ese período.
                report.filtrosDisponibles.eventos.length === 0 ? (
                  <Text style={styles.sheetHint}>Ningún evento con ventas en el período elegido.</Text>
                ) : (
                  report.filtrosDisponibles.eventos.map((opcion) => (
                    <Pressable
                      key={opcion.id}
                      style={styles.sheetRow}
                      onPress={() => {
                        setSelectedEvent({ id: opcion.id, nombre: opcion.nombre });
                        setSelectedTicketTypeId(undefined);
                        setEventPickerVisible(false);
                      }}
                    >
...
const styles = StyleSheet.create({
  ...
  hintText: { fontFamily: fonts.regular, fontSize: 12, color: colors.inkSoft, fontStyle: 'italic', marginTop: spacing.md },
