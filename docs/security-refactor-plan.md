...
> **Estado:** Etapas 0, 1, 2, 3, 4, 5 y 6 implementadas y verificadas contra Firestore Emulator real. **Refactor del módulo de seguridad completo.** Etapa 6 (retiro del modelo legacy `ApplicationUser`/subclases y de los custom claims temporales de rol): **207 passed, 0 failed, 0 skipped**, suite completa vía `npx --yes firebase-tools@13.35.1 emulators:exec --only firestore --project hoydonde-security-refactor-tests "dotnet test HoyDonde.sln"` (mismo entorno reproducible que las etapas anteriores, ver nota al final de §6, Etapa 2). El descenso de 208 a 207 tests respecto de la Etapa 5 se debe únicamente a eliminar el test que ejercitaba `SetTemporaryClaimAsync`, método que dejó de existir; la compensación por fallo de `ProvisionarAsync` (Firestore) sigue cubierta por otro test intacto. El alcance de la Etapa 2 quedó corregido antes de su commit (ver §7). Decisiones de Cliente, bootstrap y test de atomicidad cerradas (§2.1, §5, §6). Premisa: **no hay usuarios, eventos, tickets ni datos de producción que preservar** — los datos de desarrollo se pueden reiniciar libremente. Esto elimina toda la complejidad de migración/backfill/branching legacy-vs-nuevo de las versiones anteriores de este plan.
...
### Etapa 6 — Retiro del modelo legacy y de los claims temporales — **implementada y verificada**
- **Objetivo:** eliminar `ApplicationUser`/subclases, `Roles.cs`, `[Authorize(Roles=...)]` restante, el claim `role` (incluido el que la Etapa 3 asignaba temporalmente en cada sync de Cliente, §2.1 punto 7), `IJwtService` y el resto del código muerto identificado desde el diagnóstico original.
- **Componentes eliminados (15 archivos):**
  - 8 modelos legacy: `Models/ApplicationUser.cs`, `Admin.cs`, `Cliente.cs`, `Organizador.cs`, `Control.cs` (reemplazados por `Persona`+`UsuarioRol`+`ControlAsignacion`), `Roles.cs`, `UserAudit.cs`, `ISoftDeletable.cs` (sin otro consumidor).
  - 2 repositorios legacy: `Repositories/IUserRepository.cs`, `FirestoreUserRepository.cs`.
  - 1 servicio muerto: `Services/IJwtService.cs` (sin consumidor real, solo definición).
  - 4 DTOs muertos: `DTOs/LoginRequestDto.cs`, `AuthResponseDto.cs`, `AdminResetPasswordDto.cs`, `ResetPasswordDto.cs` (sin consumidor real).
  - Retirados del código sin borrar archivo: `RoleClaimType` (config JWT en `Program.cs`), `IIdentityProvider.SetTemporaryClaimAsync`/`ClearClaimAsync` y su implementación en `FirebaseIdentityProvider`, el parámetro `legacyRoleClaim` de `UserService.ProvisionarConCompensacionAsync`, la escritura de claim en `AuthService.SyncClienteAsync`, y `ClaimsUpdated` de `SyncClienteResult`/`SyncUserResponseDto`/`AuthController`.
  - Tests: `TestApplicationFactory`/`FakeAuthHandler` sin `Test-Role` ni mock de `IUserRepository` (la identidad falsa solo aporta UID/email; la autorización de tests se resuelve vía `GrantAccion` contra `IPermissionService`, sin infraestructura de rol legacy); tests de `AuthService`/`UserService`/`AuthController`/`UserController`/`TicketsController`/`BootstrapAdminCommand` adaptados para no depender de `Test-Role`, `Models.Roles`, `SetTemporaryClaimAsync` ni `ClaimsUpdated`.
- **Persistencia:** `users`/`user_audits` dejan de leerse y escribirse por completo desde cualquier flujo (no se ejecutó borrado ni migración de esas colecciones: no hay dato de valor ahí y no hay datos de producción en juego, consistente con la premisa de este plan).
- **Contrato final de autorización:** Firebase demuestra únicamente identidad (UID); Firestore vía `Usuario → UsuarioRol → Rol → RolAccion → Accion` es la única fuente de permisos, resuelta por `AccionAuthorizationHandler`/`IPermissionService` contra `[Authorize(Policy="ACCION_CODIGO")]`. No queda ningún custom claim de rol en el código ni en `IIdentityProvider`. `PersonaId` sigue siendo el único puente con `Event`/`Ticket`/`ControlAsignacion` (sin cambios de la Etapa 4). El aprovisionamiento (Admin/Organizador/Control vía `IIdentityProvider` + compensación; Cliente vía `/api/auth/sync` idempotente) y la compensación por fallo (`DeleteIdentityAsync` → `identidades_huerfanas`) quedan intactos, solo sin la escritura de claim.
- **Pruebas:** regresión completa. **Cumplido y verificado**: suite completa ejecutada contra Firestore Emulator real (Firebase CLI 13.35.1 vía `npx`, mismo entorno reproducible que las etapas anteriores) con resultado **207 passed, 0 failed, 0 skipped**.
- **Criterio de finalización:** `rg` sobre producción y tests confirma cero referencias reales a `ApplicationUser`, `Models.Admin/Cliente/Organizador/Control`, `Roles.`/`Models.Roles`, `IUserRepository`, `FirestoreUserRepository`, `UserAudit`, `user_audits`, `RoleClaimType`, `SetTemporaryClaimAsync`, `ClearClaimAsync`, `ClaimsUpdated`, `Test-Role`, `IJwtService`, `LoginRequestDto`, `AuthResponseDto`. Cero `[Authorize(Roles=...)]` reales (solo 2 menciones en comentarios históricos). Exactamente 20 acciones/policies en `Acciones.cs`, sin cambios respecto de la Etapa 5.
- **Rollback:** el más costoso de la serie, pero de bajo riesgo real porque no hay datos de producción en juego.

---

## 11. Cierre del refactor

Con la Etapa 6, el refactor del módulo de seguridad de HoyDonde? queda completo: las siete etapas (0–6) están implementadas y verificadas contra Firestore Emulator real, sin modelo legacy ni claims temporales remanentes. El estado final del módulo de seguridad —arquitectura, flujos de alta, persistencia y reglas de trabajo— se documenta de aquí en más en `CLAUDE.md`; este documento queda como registro histórico del diseño y la migración por etapas.
