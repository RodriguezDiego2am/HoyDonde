...
> **Estado:** Etapas 0, 1, 2, 3 y 4 implementadas y verificadas contra Firestore Emulator real. Etapa 4 (adaptación directa del dominio a PersonaId): **106 passed, 0 failed, 0 skipped**, misma suite completa vía `npx firebase-tools@13.35.1 emulators:exec --only firestore --project hoydonde-security-refactor-tests "dotnet test HoyDonde.sln"` (ver nota de ejecución reproducible al final de §6, Etapa 2). El alcance de la Etapa 2 quedó corregido antes de su commit (ver §7). Etapas 5–6 pendientes. Decisiones de Cliente, bootstrap y test de atomicidad cerradas (§2.1, §5, §6). Premisa: **no hay usuarios, eventos, tickets ni datos de producción que preservar** — los datos de desarrollo se pueden reiniciar libremente. Esto elimina toda la complejidad de migración/backfill/branching legacy-vs-nuevo de las versiones anteriores de este plan.
